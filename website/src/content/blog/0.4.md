...
SVGs are rendered with the **`SvgViewer`** component, built on top of the `image()` element. It rasterizes the SVG (optionally in a background thread) and caches the result, with optional color and stroke overrides:
```rust
SvgViewer::new(include_bytes!("./icon.svg"))
    .color(Color::WHITE)
    .stroke_width(2.0)
    .width(Size::px(24.))
    .height(Size::px(24.))
