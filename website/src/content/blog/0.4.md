...
For building your own inputs and editors, Freya provides a low-level text editing API. `use_editable` manages the cursor, selection, and edit history over a `paragraph()`, and you drive it by forwarding pointer and keyboard events as `EditableEvent`s. The built-in [`Input`](https://docs.rs/freya/0.4.0/freya/components/struct.Input.html) and [`CodeEditor`](https://docs.rs/freya/0.4.0/freya/code_editor/struct.CodeEditor.html) are both built on top of it. See [`text_editing.rs`](https://github.com/marc2332/freya/blob/main/examples/text_editing.rs) for a complete example.

### 🧩 Components

Components in Freya 0.4 are any data type that implements the [`Component`](https://docs.rs/freya/0.4.0/freya/prelude/trait.Component.html) trait. In practice, you define a struct with your props, derive `PartialEq` (so the framework can diff them), and implement `render`:
...
<div align="center">
  <img src="/blog/0.4/transform.png">
</div>
...
The performance overlay renders a small panel on top of your app with live metrics. Enabled by default on debug builds. Toggle it with `Ctrl+Shift+P` (or `Cmd+Shift+P` on macOS):
...
![Sanctum Control Panel Screenshot](/blog/0.4/sanctum-2.png)
